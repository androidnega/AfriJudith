...
(function () {
    ...
    if (toggle && nav) {
        ...
        // Scroll lock: just toggle a class on <html> and <body>. The CSS
        // pins `overflow: hidden` on both so the page can't scroll behind
        // the drawer, but we deliberately do NOT switch <body> to
        // `position: fixed` — doing so on iOS can swallow tap → navigate
        // on links inside a transformed drawer and was the cause of "menu
        // opens but links don't go anywhere".
        const setOpen = (open) => {
        ...
        // Drawer link → close the drawer, then let the browser's default
        // navigation take over. We intentionally don't touch the event:
        // no preventDefault, no stopPropagation, no scroll tweaks. The
        // browser handles the link, the drawer just hides itself on the
        // way out.
        ...
        // overflow lock + transform aren't left dangling on resize.
